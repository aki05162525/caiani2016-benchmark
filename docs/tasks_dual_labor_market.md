...
* [x] 期待賃金の更新（観測）は type 別平均賃金から2系列で更新する
...
* [x] 範囲チェック（δ∈(0,1), A>0, φ_min>0, φ_min<φ_max, etc.）
* [x] ρ の危険域（|ρ|小）警告 or 禁止
...
* [x] 支払不能→解雇→再求人が type 市場に戻る（混線しない）
* [x] R市場求人にN家計が採用されない（市場IDで担保）
* [x] computeLaborDemand（7クラス）の一貫性（少なくとも主要2クラスで先に通す→残りへ展開）
